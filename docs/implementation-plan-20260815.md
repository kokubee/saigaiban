...
### 2026-08-24 — 生活目的別入口と印刷表示

市区町村ページのカテゴリ一覧とは別に、利用者の目的から選べる入口を追加した。OpenNaviのtaxonomyまたは取得済み場所に存在するカテゴリだけをリンクし、存在しない情報をあるように見せない。

- 避難・安全、水・給水、食事・物資、トイレ、給油・店、医療、洗濯・入浴を目的別に表示
- 既存のカテゴリフィルタと検索は維持し、目的別入口から同じ読み取りAPIへ遷移
- 印刷時はナビゲーション、検索、投稿操作、オフライン保存操作を隠し、カードを印刷しやすくする
- 固定テストで目的別リンクの出現条件と印刷用CSSを確認
