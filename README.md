...
市区町村ページでは、場所APIの `generated_at` を「OpenNavi場所台帳の取得時刻」として表示します。
24時間超は要再確認、72時間超は古い可能性ありとして扱い、台帳の更新時刻を営業中・開設中・安全の
判定には使いません。`/health` はこの依存APIも確認し、取得できない場合は 502 と
`dependencies.opennavi.ok=false` を返します。API障害時に推測で古い状態を「現在」として出さず、
地域別のOpenNavi公式ハブへ退避するための契約です。
...
災害板は、訪問しただけでページを自動保存するPWAではありません。市区町村ページの「この地域を端末に保存」を利用者が押したときだけ、公開場所カードと保存時点の構造化された報告をIndexedDBへ保存します。

- 保存した地域は `/a/<area-slug>/offline` で読み取り専用に確認できます。
- 保存日時と「最新ではない可能性」を表示します。営業・開設・安全は保証しません。
- オフライン中の投稿、通報、確認、自動送信は完了扱いにしません。
- Service Workerはmanifest・アイコン・オフラインシェルだけをキャッシュし、動的な町ページ・場所詳細・API・POSTはキャッシュしません。
- モデレーションの取り消しは通信断中には伝わりません。通信が戻ったら通常ページを開いて確認してください。
- オンラインで保存情報を開いたときは、公開リビジョンを照合し、更新や非表示変更があれば古い保存内容を表示しません。

詳細な保存項目と境界は [PWA / オフライン保存契約](docs/pwa-offline-contract.md) を正本とします。
